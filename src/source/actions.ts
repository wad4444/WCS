...
function getAtom() {
    ...
    if (!handler) logError(`Attempted to dispatch an action before server handler initialization.`);

    return handler.__WCS_Atom;
}

export function setCharacterData(CharacterIndex: string, CharacterData: CharacterData) {
    MutateAtom(getAtom(), (Draft) => {
        Draft.set(CharacterIndex, CharacterData);
    });
}

export function patchCharacterData(CharacterIndex: string, Patch: Partial<CharacterData>) {
    const atom = getAtom();
    const state = atom();
...
export function setStatusData(CharacterIndex: string, Id: string, Data: StatusData) {
    MutateAtom(getAtom(), (Draft) => {
...
export function patchStatusData(CharacterIndex: string, Id: string, Patch: Partial<StatusData>) {
    const atom = getAtom();
    const state = atom();
...
export function setSkillData(CharacterIndex: string, Name: string, Data: SkillData) {
    MutateAtom(getAtom(), (Draft) => {
...
export function patchSkillData(CharacterIndex: string, Name: string, Patch: Partial<SkillData>) {
    const atom = getAtom();
    const state = atom();
    const characterData = state.get(CharacterIndex);
